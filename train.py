...
def parse_args():
    ...
    # yanjie ze
    parser.add_argument("--action_space", type=str, choices=["xy", "xyz", "xyzw"], default="xyzw")
...
def evaluate(env, agent, video, num_episodes, L, step, args):
    ...
    def run_eval_loop(sample_stochastically=True):
        ...
        if args.use_wandb:
            wandb.log({"eval/sucess_rate": success_rate}, step)
            wandb.log({"eval/episode_rewards":mean_ep_reward}, step)
...
def main():
    ...
    if args.use_wandb:
        ...
        print(colored(f"[wandb] project:{args.wandb_project} | group:{args.wandb_group} | name:{args.wandb_name}", "green"))
    ...
    env = make_env(
		...
		n_substeps=20,
		frame_stack=1,
		image_size=args.pre_transform_image_size,
		cameras="dynamic",
		render=False, # Only render if observation type is state
		observation_type="image", # state, image, state+image
